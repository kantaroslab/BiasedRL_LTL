...
def main(output_folder, writer):
    ...
    epsilon = float(config['pure_dqn_epsilon'][0])
    epsilon_min = float(config['pure_dqn_epsilon'][1])
    epsilon_decay_steps = config['pure_dqn_epsilon'][2]
    ...
    runtime_rl = config['RL_RUNTIME']
    biasnn_time = config['BIASNN_RUNTIME']
    total_allowed_runtime_dqn = float(runtime_rl) +float(biasnn_time)

    while (time.time() - init_time)/60 <= total_allowed_runtime_dqn:
        ...
        logging.info("Episode:{} | R:{:.1f} | Epi_Loss:{:.2f} | 1-ε:{:.2f} | Time:{:.1f} min".format(cur_epi, discount_episode_reward, dqn_loss, 1-epsilon, epi_time/60))
